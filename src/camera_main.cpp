...
void overtake_left(int16_t);
void overtake_right(int16_t);
...
//		}
		...
		if(UPDATE_SERVO){
			...
			//			if(TCount > 6 && TCount < 13){
			////				if(midpoints[midpoint_ending_index] > WIDTH/2){
			//					overtake_left(count);
			////				}
			////				else{
			////					overtake_right(count);
			////				}
			//				//				overtake_right();
			//				//				Runner.ServoErr = 10;
			//			}
...
//				count = 190;
			...
			int32_t ideal_servo_degree = uint16_t(975 + P*Runner.ServoErr + D*(Runner.ServoErr - Runner.ServoPrevErr));
...
//			obj_distance = Chase.distanceControl();
//			Chase.update_Usensor();
//		}
//		count -= obj_distance;
...
//		if(count < 110) count = 120;
		...
		if(System::Time() - t > 2000 && abs(encoder->GetCount()) < 4 && run){
			motor->SetPower(0);
			break;
		}
		...
		/*
		if(!reached_triangle){
			area = check_triangle1(image, absolute_midpoint_ending_index, WIDTH/2);
			if (area == 0){
				area = check_triangle1(image, absolute_midpoint_ending_index, WIDTH/2 - 3);
			}
			if (area == 0){
				area = check_triangle1(image, absolute_midpoint_ending_index, WIDTH/2 + 3);
			}
...
void overtake_left(int16_t count){
...
void overtake_right(int16_t count){
	if(count <= 150){
		servo->SetDegree(550);
		System::DelayMs(600);
	}
