...
int16_t ChaseMethod::distanceControl(){
	...
	distancePIDOutput = distance_kp*distanceError*distanceError + distance_kd*(distanceErrorPrev-distanceError);

	if(objDistance <200) {
		distancePIDOutput = TOO_CLOSE_FROM_FRONT_CAR;
		return 50;
	}

	if(distanceError>0){
		return distancePIDOutput / 100;
	}
	else return -1* distancePIDOutput / 100;
}
